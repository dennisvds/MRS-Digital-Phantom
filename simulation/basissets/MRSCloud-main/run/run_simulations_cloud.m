function basis = run_simulations_cloud()
...
disp(['Deleting files from: ', save_dir]);
delete([save_dir,'/*']); 
...
for iii = 1:length(metablist)
    ...
    metab_name = metablist{iii};
    fprintf('--- Simulating %s at TE %d ms ---\n', metab_name, TE);
...
for ss = 1:length(subspec)
    outfile   = [save_dir '/LCModel_' vendor '_' sequence '_' localization '_TE' num2str(TE) '' subspecName{ss} '' '.BASIS'];
